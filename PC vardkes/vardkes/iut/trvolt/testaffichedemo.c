...
char message[16]="bonjour geii";   //Message à Afficher
...
XBR0 = 0x01;	// XBAR0: Initial Reset Value	
...
void interuption (void) interrupt 7
{
switch (SMB0DAT)
{
case 0x08 :
...
void SM_SEND (char chip_select, unsigned int byte_adress, char out_byte)
{
   while (SM_BUSY) ;
   SM_BUSY=1;
   SMB0CN=0x44;
   ...
   BYTE_NUMBER = 2 ;
   COMMAND = (chip _select | WRITE) ;
   HIGH_ADD = (byte_address >> 8) & 0x00FF) ;
   LOW_ADD = (byte_address & 0x0FF) ;
