<%
lePathRoot = Session("PortalPath") & Session("lePathRoot")
 ...
 if (document.uplForm.f_NewFile.value + "" != "") {
  ...
  if (myError == 1) {
   ...
   if (myError == 1) {
    ...
    myHref += "&h_formback=" + document.imgForm.h_formback.value;
    myHref += "&h_imgchange=" + document.imgForm.h_imgchange.value;
...
function SetBgImage(inIMG) {
 ...
 window.opener.document.images['<%Response.Write leImgChange%>'].src = inIMG;
 window.opener.<%Response.Write leFormBack%>.value = inIMG.replace(lHost, "");
 self.close();
};
</script>
</head>
    ...
    <tr>
     ...
     <td class="Text">
      ...
      <input type="text" name="f_NewFolder" class="Text" style="width:220;" onBlur="jsTrim(this);CheckFolderName();">&nbsp;<input type="button" name="Add" value="Ajouter" class="Text" onClick="AddFolder();">
        ...
        <input type="hidden" name="h_page" value="cand">
 ...
 If IsArray(aFile) Then
  ...
  For i = tdDisp To nbCol - 1
   If indx =< UBound(aFile, 2) Then
    ...
    leStr = leStr & "      <a href=""javascript:SetBgImage('" & lePathEnCours & aFile(0, indx) & "');"" border=""0"" onMouseOver=""window.status='Insérer l\'image';return true;"" onMouseOut=""window.status=' ';return true;"">" & vbCrLf
    leStr = leStr & "      <img src=""../Img/Uploader/img_back.gif"" width=""20"" height=""21"" border=""0"" title=""Insérer l'image"">" & vbCrLf
