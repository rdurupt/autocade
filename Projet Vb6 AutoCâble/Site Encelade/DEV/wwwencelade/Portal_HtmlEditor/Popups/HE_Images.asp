<%
lePathRoot = Session("PortalPath") & "Portal_Upload/Images/"
 ...
 if (document.uplForm.f_NewFile.value + "" != "") {
  ...
  if (myError == 1) {
   ...
   if (myError == 1) {
    ...
    myHref += "&mode=" + document.imgForm.mode.value;
    myHref += "&foo=" + document.imgForm.foo.value;
    myHref += "&f_win=" + document.imgForm.f_win.value;
    myHref += "&f_form=" + document.imgForm.f_form.value;
    myHref += "&_display=" + document.imgForm._display.value;
...
function SetImage(inIMG) {
 var foo = window.opener.des_<%Response.Write Request("foo")%>;
 var sel = foo.document.selection;
 if (sel != null) {
  var rng = sel.createRange();
  if (rng != null) {
   rng.execCommand("InsertImage", false, inIMG);
  }
 }
 foo.focus();
 self.close();
...
<%
 If leMode = "editor" Then
%>
 var foo = eval("window.opener.des_<%Response.Write Request("foo")%>");
 foo.document.body.background = inIMG;
<%
 Else
%>
 var foo = window.opener.JS_HE.<%Response.Write Request("f_win")%>;
 foo.document.<%Response.Write Request("f_form")%>.background.value = inIMG;
<%
 End If
%>
 self.close();
   ...
   <table border="0" width="98%" cellpadding="0" cellspacing="5" style="table-layout:fixed;">
    <tr>
     <td class="Text">
      ...
      <input type="hidden" name="h_page" value="img">
      <input type="hidden" name="mode" value="<%Response.Write leMode%>">
      <input type="hidden" name="h_mode" value="">
      <input type="hidden" name="_display" value="<%Response.Write leDisp%>">
      <input type="hidden" name="foo" value="<%Response.Write Request("foo")%>">
      <input type="hidden" name="f_win" value="<%Response.Write Request("f_win")%>">
      <input type="hidden" name="f_form" value="<%Response.Write Request("f_form")%>">
 ...
 If IsArray(aFile) Then
  ...
  For i = tdDisp To nbCol - 1
   If indx =< UBound(aFile, 2) Then
    ...
    If leMode = "editor" Then
     leStr = leStr & "      <a href=""javascript:SetImage('" & lePathEnCours & aFile(0, indx) & "');"" border=""0"" onMouseOver=""window.status='Insérer l\'image';return true;"" onMouseOut=""window.status=' ';return true;"">" & vbCrLf
     leStr = leStr & "      <img src=""../Img/Uploader/img.gif"" width=""20"" height=""21"" border=""0"" title=""Insérer l'image"">" & vbCrLf
     leStr = leStr & "      </a>" & vbCrLf
    End If
    If (leMode = "editor" And leDisp <> "body") Or leMode <> "editor" Then
  ...
  For i = indx To Ubound(aFile, 2)
   ...
   leStr = leStr & "     <td width=""20"" align=""center"" valign=""top"">" & vbCrLf
   If leMode = "editor" Then
    leStr = leStr & "      <a href=""javascript:SetImage('" & lePathEnCours & aFile(0, i) & "');"" border=""0"" onMouseOver=""window.status='Insérer l\'image';return true;"" onMouseOut=""window.status=' ';return true;"">" & vbCrLf
    leStr = leStr & "      <img src=""../Img/Uploader/img.gif"" width=""20"" height=""21"" border=""0"" title=""Insérer l'image"">" & vbCrLf
    leStr = leStr & "      </a>" & vbCrLf
   End If
   If (leMode = "editor" And leDisp <> "body") Or leMode <> "editor" Then
