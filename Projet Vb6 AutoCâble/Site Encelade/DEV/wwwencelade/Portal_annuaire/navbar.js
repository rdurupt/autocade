...
function NavBar(width) {
  ...
  this.setImages   = navBarSetImages;
...
function navBarSetImages(BgImage,OnImage, OffImage) {

  if (!this.created) {
    if (BgImage!="") this.BgImg = 'Background="' + BgImage + '" ' ;
    if (OnImage!="") this.OnImg = 'Background="' + OnImage + '" ';
    if (OffImage!="") this.OffImg = 'Background="' + OffImage + '" ';
  }
}
...
function navBarCreate() {
  ...
  // Border à 1 donne du relief au bouton
  for (i = 0; i < this.menus.length; i++) {
    norm = '<table border=0 '+ this.BgImg + ' cellpadding=' + this.padding
	 	 + ' cellspacing=0 '
    ...
    high = '<table border=0 '+ this.BgImg + ' cellpadding=' + this.padding
         + ' cellspacing=0 '
         + (this.menus[i].hdrWidth > 0 ? ' width=' + this.menus[i].hdrWidth : '')
         + '><tr><td '+ this.OnImg
         + (this.menus[i].hdrWidth == 0 ? ' nowrap=1' + this.menus[i].hdrWidth : '')
    ...
    if (isMinIE4) {
      str += '<div id="navBar' + this.index + '_head' + i + '"'
          ...
          +  ' style="position:absolute;">';
      if (isMinIE5_5)
        str += '<table cellspacing=0 border=0 width="100%" height="100%"><tr><td></td></tr></table>';
      str += '</div>\n';
    }
  }
  ...
  if (isMinIE4) {
    ...
    str = '<div id="navBar' + this.index + '"'
        + ' style="position:absolute;center:0px;top:0px;">\n'
        + str
        + '</div>\n';
